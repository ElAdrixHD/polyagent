...
def main() -> None:
    ...
    if config.tmc_enabled:
        ...
        logger.info(f"  Max entry ask: {config.tmc_max_entry_ask}")
        logger.info(f"  Min cheap ask: {config.tmc_min_cheap_ask}")
        logger.info(f"  Max tight ratio: {config.tmc_max_tight_ratio}")
